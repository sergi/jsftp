...
COMMANDS.forEach(function(cmd) {
  ...
  Cmds[cmd] = function() {
    ...
    this._execute(completeCmd.trim(), callback);
  };
});
...
Ftp.prototype._socketOnTimeout = function() {
  this.emit("timeout", errString);
  this.pipeline.pause();
  this.destroy();
};

Ftp.prototype._socketOnError = function(err) {
  var errString = "Error on ftp socket: " + err;
  this.emit("error", errString);
  this.pipeline.pause();
};

Ftp.prototype._socketOnConnect = function() {
...
Ftp.prototype._createSocket = function(port, host) {
  ...
  socket.setTimeout(this.timeout || TIMEOUT,
    this._socketOnTimeout.bind(this));
  socket.on("error", this._socketOnError.bind(this));
 ...
 * @return void
 */
Ftp.prototype._execute = function(action, callback) {
 ...
 */
Ftp.prototype.parse = function(action) {
  var response = action[0];
  var command = action[1];
  var cb = command[1];

  if (!response)
    cb(new Error("FTP response not defined"));
 ...
 * case, an optional callback can be provided, which will return `true` in case
 * of successful transfer or receive the error in case the operation was not
 * successful.
 *
  ...
  if (arguments.length === 2) {
    ...
    this.getGetSocket(remotePath, function(err, socket) {
      ...
      function onError(err) {
        console.error("There was an error transferring the file.");
        callback(err);
      }
      ...
      socket.on('end', function() { callback(null); });
...
Ftp.prototype.getGetSocket = function(path, callback) {
  ...
  this.getPasvSocket(function(err, socket) {
    ...
    self._execute("retr " + path, cmdCallback);
  });
};
  ...
  if (from instanceof Buffer) {
    ...
    fs.exists(from, function(exists) {
      ...
      self.getPutSocket(to, function(err, socket) {
        if (!err) {
          var origin = fs.createReadStream(from, { bufferSize: 4 * 1024});
          origin.pipe(socket);
        }
      }, callback);
    });
...
Ftp.prototype.getPutSocket = function(path, callback, doneCallback) {
  ...
  this.getPasvSocket(function(err, socket) {
    ...
    self._execute("stor " + path, putCallback);
  });
};
...
Ftp.prototype.getPasvSocket = function(callback) {
  ...
  callback = once(callback || function() {});
  this._execute("pasv", function(err, res) {
    if (err) return callback(err);
