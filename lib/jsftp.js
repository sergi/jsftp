...
function runCmd(name, ...params) {
  ...
  let completeCmd = [name, ...params].join(" ");
  this.execute(completeCmd.trim(), callback);
}

function ignoreByCode(response) {
  // 220 -> Service Ready for new user code
  if ([220].indexOf(response.code) > -1) {
    return false;
  }
}

function isMark(response) {
  return !response.isMark;
}
...
function Ftp(cfg) {
  ...
  this.commandQueue = queue(worker, 1);
  this.commandQueue = [];
...
Ftp.prototype.put = function(from, destination, callback) {
  ...
  if (from instanceof Buffer) {
    ...
    stat(from, (err, stats) => {
      ...
      putReadable(createReadStream(from), destination, totalSize);
    });
