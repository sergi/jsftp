...
Ftp.prototype._createSocket = function(port, host, firstAction=NOOP) {
  ...
  if (this.cmdSubject$) { this.cmdSubject$.dispose(); }

  this.cmdSubject$ = new Rx.Subject();

  let firstCmd$ = this.cmdSubject$.take(1);
  let restCmd$ = this.cmdSubject$.skip(1);

  //firstCmd$.subscribe(v=>console.log('F',v))
  //restCmd$.subscribe(v=>console.log('R', v))

  let validResponses$ = RxNode
    .fromStream(this.pipeline)
    .filter(res => [220].indexOf(res.code) === -1);

  validResponses$.subscribe(v=>console.log('V', v))

  this.cmdResponsePair$ = Rx.Observable.zip(
    Rx.Observable.merge(firstCmd$, restCmd$),
    validResponses$,
    (cmd, res) => [cmd, res]
  ).subscribe(pair => {
    //console.log(pair);
...
Ftp.prototype.list = function(path, callback) {
  ...
  this.getPasvSocket((err, socket) => {
    ...
    socket.on('data', data => listing += data.toString('binary'));
