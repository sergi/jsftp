...
/**
 * Calculates the passive port from an FTP PASV response text
 *
 * @nosideeffects
 * @throws {Error}
 * @param {String} text Response from the FTP server
 * @return {{host: string, port: number}}
 */
Ftp.getPasvPort = function(text, callback) {
...
Ftp.prototype.get = function(remotePath, localPath, callback) {
  if (typeof localPath === 'function') {
    this.getRETRSocket(remotePath, once(localPath || NOOP));
    return;
  }

  callback = once(callback || NOOP);
  var self = this;
  var finalCallback = function(err, socket) {
    if (err) return callback(err);
  ...
  this.getRETRSocket(remotePath, finalCallback);
};
 ...
 * as a parameter
 */
Ftp.prototype.getRETRSocket = function(path, callback) {
  var self = this;
  callback = once(callback);
...
Ftp.prototype.put = function(from, to, callback) {
  ...
  function putReadable(from, to, totalSize, callback) {
    ...
    self.getSTORSocket(to, function(err, socket) {
      if (err) return;
      from.pipe(socket);
    }, callback);
  }
  ...
  if (from instanceof Buffer) {
    this.getSTORSocket(to, function(err, socket) {
      if (!err) {
        socket.once('close', function(hadError) {
          ...
          self.emitProgress({
            ...
            // (http://nodejs.org/api/buffer.html#buffer_buf_length)
            totalSize: from.length
          });
        });
        socket.end(from);
      }
...
Ftp.prototype.getSTORSocket = function(path, callback, doneCallback) {
  if (!callback) {
...
Ftp.prototype.getPasvSocket = function(callback) {
  ...
  this.execute('pasv', function(err, res) {
    ...
    var options;
    try {
      options = Ftp.getPasvPort(res.text);
    } catch ( e ) {
      return callback(e);
    }
