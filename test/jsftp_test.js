...
describe("jsftp test suite", function() {
  ...
  describe("FTP PUT command", () => {
    ...
    it("test put a big file stream", function(next) {
      ...
      ftp.getPutSocket(Buffer.from(data), remotePath, function(err, res) {
        assert.ok(!err, err);
    ...
    it("Test that streaming PUT (STOR) stores a file properly", function(next) {
      ...
      originalFileStream.on("error", function(err) {
        throw new Error(err);
      });

      ftp.getPutSocket(
        originalFileStream,
        getRemoteFixturesPath("testfile.txt.bak"),
        (err, socket) => {
          assert.ok(!err);
          assert.equal(
            Fs.readFileSync(getLocalFixturesPath("testfile.txt.bak")),
    ...
    it("Test that streaming PUT (STOR) fails when a file is not present", function(
      ...
      ftp.put(
        ...
        getRemoteFixturesPath("newFile.txt"),
        function(err, socket) {
          assert.ok(err);
          next();
        }
