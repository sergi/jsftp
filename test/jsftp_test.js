...
describe("jsftp test suite", function() {
  ...
  it("test invalid password", function(next) {
    ftp.auth(options.user, options.pass + "_invalid", function(err, data) {
  ...
  it("test passing a dir instead of file path to put should callback with error", function(
    next
  ) {
    var localUploadPath = ".";
  ...
  it("test streaming put", function(next) {
    ...
    ftp.put(readStream, filePath, function(hadError) {
      ...
      var uploadedFileSize = Fs.statSync(getLocalFixturesPath(remoteFileName))
        .size;
  ...
  it("Test that streaming GET (RETR) retrieves a file properly", function(
    next
  ) {
    var path = getLocalFixturesPath("testfile.txt");
    var originalData = Fs.readFileSync(path);
    ftp.getGetSocket(getRemoteFixturesPath("testfile.txt"), function(
      err,
      readable
    ) {
      assert.ok(!err);
  ...
  it("Test that streaming GET (RETR) fails when a file is not present", function(
    next
  ) {
  ...
  it("Test that streaming PUT (STOR) stores a file properly", function(next) {
    ...
    var originalData = Fs.createReadStream(
      getLocalFixturesPath("testfile.txt")
    );
    originalData.pause();

    ftp.getPutSocket(getRemoteFixturesPath("testfile.txt.bak"), function(
      err,
      socket
    ) {
      assert.ok(!err);
  ...
  it("Test that streaming PUT (STOR) fails when a file is not present", function(
    next
  ) {
  ...
  it.skip("test listing a folder containing special UTF characters", function(
    next
  ) {
